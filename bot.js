...
// Миграция: добавляем first_name и last_name в members, если столбцов нет
try {
...
bot.on('new_chat_members', (ctx) => {
    ctx.message.new_chat_members.forEach((member) => {
        ...
        db.prepare(
            `INSERT
            OR REPLACE INTO members (id, first_name, last_name) VALUES (?, ?, ?)`,
...
bot.on('callback_query', async (ctx) => {
    ...
    const yes = db
    .prepare(`SELECT first_name, last_name
              FROM votes v
                       JOIN members m ON v.user_id = m.id
              WHERE v.vote = 'yes'
                AND v.session_id = ?`)
    .all(sessionId)
    .map((r) => `${ r.first_name }${ r.last_name ? ` ${ r.last_name }` : '' }`);
...
cron.schedule('0 */2 * * *', async () => {
    ...
    for (const session of activeSessions) {
        ...
        const members = db.prepare(`SELECT id, first_name, last_name
                                    FROM members`).all();
        
        const notVotedMembers = members.filter((m) => !votedUserIds.includes(m.id));
        const mentions = notVotedMembers
        .map((m) => m.first_name ? `${ m.first_name }${ m.last_name ? ` ${ m.last_name }` : '' }` : '')
        .filter((s) => s.length > 0)
        .join(', ');
